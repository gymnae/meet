...
(() => {
    ...
    const bgOf = (el) => {
        for (let e = el; e; e = e.parentElement) {
            ...
            if (alpha(c) > 0.5) return c;
        }
        return 'rgb(10, 11, 16)';
    };
    const ratio = (a, b) => { const x = lum(a), y = lum(b); return (Math.max(x, y) + 0.05) / (Math.min(x, y) + 0.05); };
    const out = { smallText: new Set(), lowContrast: new Set(), uppercase: new Set(), smallTargets: new Set(), unnamed: new Set() };

    document.querySelectorAll('body *').forEach((el) => {
        if (el.closest('.px-bg, svg')) return;
        ...
        if (el.matches('button, a[href], input:not([type=file]), [role=button]')) {
            if (r.width < 44 || r.height < 44) out.smallTargets.add(`${key} ${Math.round(r.width)}x${Math.round(r.height)}`);
